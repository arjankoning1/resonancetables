subroutine readthermal(Z, A, Liso, Riso, type, flagav)
!
!-----------------------------------------------------------------------------------------------------------------------------------
...
!
  xs = 0.
  dxs = 0.
  if (.not. flagav .and. (type >= 3 .and. type <= 6)) then
...
!
  ...
  dxs = 0.
  ref = ''
  if ((.not.flagav .and. (type == 2 .or. type == 3 .or. type == 6)) .or. type == 4) then
...
!
  xs = 0.
  dxs = 0.
  if (flagav .and. type == 4 .and. A == 0 .and. Liso == 0 .and. Riso == -1) then
...
!
  xs = 0.
  dxs = 0.
  if (type == 4 .or. (.not.flagav .and. (type == 3 .or. type == 5 .or. type == 6))) then
...
!
  xs = 0.
  dxs = 0.
  if (.not.flagav .and. (type == 4 .and. Riso == -1)) then
    do lib = 1, numndlib
  ...
  if (shin) then
    ...
    if (lexist .and. Liso == 0) then
      ...
      do
        ...
        if (recom > 0) cycle
        mxw=index(line,'MXW') 
        mxw=max(mxw,index(line,'AV') )
        mxw=max(mxw,index(line,'SPA') )
        if (mxw == 0 .and. flagav) cycle
        sub=line(1:12)
        author=line(21:44)
