...
!
  ...
  do isource = 1, Nsource
    sourcefile(isource)=trim(macspath)//trim(dir)//'all/'//trim(msource(isource))//'.macs'
    ifile = 10 + isource
  ...
  nucfile=trim(macspath)//trim(dir)//'all/ng.macs'
 ...
 &     refsave(k), Nexpsave(k), nuclide, avsave(k)
    macsfile=trim(macspath)//trim(dir)//'nuc/'//trim(nuclide)//'.macs'
